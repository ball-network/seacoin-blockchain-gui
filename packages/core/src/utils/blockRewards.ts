...
const MOJO_PER_SEA = new BigNumber('1000000000000');
...
export function calculatePoolReward(height: number): BigNumber {
  if (height === 0) {
    return MOJO_PER_SEA.times(3_000_000).times(POOL_REWARD)
  }
  return MOJO_PER_SEA.times(calculateReward(height)).times(POOL_REWARD);
}

export function calculateBaseFarmerReward(height: number): BigNumber {

  if (height === 0) {
    return MOJO_PER_SEA.times(3_000_000).times(FARMER_REWARD)
  }
  return MOJO_PER_SEA.times(calculateReward(height)).times(FARMER_REWARD);
}

export function calculateStakeReward(height: number): BigNumber {
  return MOJO_PER_SEA.times(calculateReward(height)).times(STAKE_REWARD);
}
