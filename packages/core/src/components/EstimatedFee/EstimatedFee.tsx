import { useGetFeeEstimateQuery } from '@sea-network/api-react';
...
import mojoToSeaLocaleString from '../../utils/mojoToSeaLocaleString';
import Fee from '../Fee';
...
export enum FeeTxType {
  walletSendXSEA = 'send_xsea_transaction',
...
export default function EstimatedFee(props: FeeProps) {
  ...
  const formattedEstimates: FormattedEstimate[] = useMemo(() => {
    ...
    return estList.map((estimate: number, i: number) => {
      const formattedEstimate = mojoToSeaLocaleString(estimate, locale);
  ...
  useEffect(() => {
    if (formattedEstimates && inputType === 'dropdown') {
      ...
      if (selectedTime) {
        ...
        if (estimate) {
          const xseaFee = mojoToSeaLocaleString(estimate.estimate, 'en-US');
          setSelectedValue(estimate.formattedEstimate);
          setValue(name, xseaFee);
        }
      }
    }
