import { defaultPlotter, PlotterName } from '@sea-network/api';
import type { Plotter, PlotterMap } from '@sea-network/api';
import { useGetPlottersQuery } from '@sea-network/api-react';
import { CardStep, Select, StateColor } from '@sea-network/core';
...
export default function PlotAddChoosePlotter(props: Props) {
  ...
  function getDisplayablePlotters(p: PlotterMap<PlotterName, Plotter>): PlotterName[] {
    ...
    displayablePlotters.sort((a, b) => (a === PlotterName.SEAPOS ? -1 : a.localeCompare(b)));
    ...
    <CardStep step={step} title={<Trans>Choose Plotter</Trans>}>
      <Typography variant="subtitle1">
        <Trans>
          ...
          the default Sea Proof of Space plotter. If unsure, use the default Sea Proof of Space plotter.
        </Trans>
      </Typography>
