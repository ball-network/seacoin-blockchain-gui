import { Button, Flex, useOpenDialog, seaToMojo } from '@sea-network/core';
...
export default function WalletConnectOfferPreview(props: WalletConnectOfferPreviewProps) {
  ...
  async function handleShowPreview() {
    ...
    if (offerBuilderData) {
      // use new fee value
      const feeSea = offerBuilderData.offered.fee?.[0]?.amount;
      if (feeSea) {
        const feeMojos = feeSea ? seaToMojo(feeSea).toFixed() : '0';
        onChange({
          ...values,
          fee: feeMojos,
        });
