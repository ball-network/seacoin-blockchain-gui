import type { Wallet, CATToken } from '@sea-network/api';
import { WalletType } from '@sea-network/api';
import { useGetCatListQuery, useGetWalletsQuery } from '@sea-network/api-react';
import { Select, useCurrencyCode } from '@sea-network/core';
...
type BuildAssetSelectorListParams = {
  ...
  seaCurrencyCode: string;
};

function buildAssetSelectorList(params: BuildAssetSelectorListParams): WalletOfferAssetSelection[] {
  const { wallets, catList, rows, otherRows, selectedWalletId, seaCurrencyCode } = params;
