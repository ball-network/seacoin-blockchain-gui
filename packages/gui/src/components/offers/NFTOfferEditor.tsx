...
import { useCreateOfferForIdsMutation, useGetWalletBalanceQuery } from '@sea-network/api-react';
import {
  Amount,
  AmountProps,
  Back,
...
/*       An NFT offer consists of a single NFT being offered for XSEA/CAT      */
/* ========================================================================== */
          ...
          <Box style={{ position: 'relative', top: '20px' }}>
            <TooltipIcon>
              <Trans>
                ...
                recommended minimum fee is 0.000005 XSEA (5,000,000 mojos)
              </Trans>
            </TooltipIcon>
          </Box>
        </Flex>
    ...
    if (
      ...
      switch (tokenWalletInfo.walletType) {
        case WalletType.STANDARD_WALLET:
          balanceString = mojoToSeaLocaleString(walletBalance.spendableBalance, locale);
          balance = mojoToSea(walletBalance.spendableBalance);
          break;
        case WalletType.CAT:
...
/* ========================================================================== */
/*                              NFT Offer Editor                              */
/*           Currently only supports a single NFT <--> XSEA/CAT offer          */
/* ========================================================================== */
...
function buildOfferRequest(params: NFTBuildOfferRequestParams) {
  ...
  const baseMojoAmount: BigNumber =
    tokenWalletInfo.walletType === WalletType.CAT ? catToMojo(tokenAmount) : seaToMojo(tokenAmount);
  const mojoAmount = exchangeType === NFTOfferExchangeType.NFTForToken ? baseMojoAmount : baseMojoAmount.negated();
  const feeMojoAmount = seaToMojo(fee);
