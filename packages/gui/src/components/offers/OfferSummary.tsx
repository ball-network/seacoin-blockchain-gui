...
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToSea, mojoToCAT } from '@sea-network/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['xsea', 'txsea'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToSea(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['xsea', 'txsea'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToSea(takerEntries[0][1])
