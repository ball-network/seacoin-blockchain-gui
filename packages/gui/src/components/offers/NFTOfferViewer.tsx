import { OfferSummaryRecord, OfferTradeRecord } from '@sea-network/api';
import { useCheckOfferValidityMutation, useGetWalletsQuery } from '@sea-network/api-react';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] = useMemo(
    () =>
      Object.keys(summaryData).map((key) => {
        ...
        if (['xsea', 'txsea'].includes(key.toLowerCase())) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.SEA: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const xseaMakerFee = mojoToSea(makerFee);

    return {
      ...calculateNFTRoyalties(
        amount,
        parseFloat(xseaMakerFee),
  ...
  const overrideNFTSellerAmount =
    exchangeType === NFTOfferExchangeType.TokenForNFT
      ? assetType === OfferAsset.SEA
        ? seaToMojo(nftSaleInfo?.nftSellerNetAmount ?? 0)
