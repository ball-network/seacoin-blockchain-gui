import { OfferTradeRecord } from '@sea-network/api';
import {
  ButtonLoading,
  CopyToClipboard,
  DialogActions,
...
const testnetDummyHost = 'offers-api-sim.seacoin.vip';
...
enum HashgreenErrorCodes {
  ...
  MARKET_NOT_FOUND = 50_029, // Pairing doesn't exist e.g. XSEA/RandoCoin
...
async function postToHashgreen(offerData: string, testnet: boolean): Promise<string> {
  ...
  if (statusCode === 200) {
    ...
    if (testnet) {
      return 'https://www.seacoin.vip/offers';
    }
...
async function postToSpacescan(offerData: string, testnet: boolean): Promise<{ viewLink: string; offerLink: string }> {
  ...
  const requestOptions = {
    ...
    port: 443,
    path: `/api/offer/upload?coin=${testnet ? 'txsea' : 'xsea'}&version=1`,
  };
  const requestHeaders = {
    'Content-Type': 'application/json',
  };
