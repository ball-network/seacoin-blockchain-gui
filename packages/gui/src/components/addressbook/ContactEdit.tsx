...
export default function ContactEdit() {
  ...
  function handleSubmit(data) {
    ...
    filteredAddresses.forEach((entry) => {
      try {
        if (entry.address[4] === '1') {
          if (entry.address.slice(0, 4).toLowerCase() !== 'xsea') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
        } else if (entry.address[5] === '1') {
          if (entry.address.slice(0, 5).toLowerCase() !== 'txsea') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
    ...
    filteredStakeAddresses.forEach((entry) => {
      try {
        if (entry.address[10] === '1') {
          if (entry.address.slice(0, 10).toLowerCase() !== 'dpos:xsea:') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
    ...
    filteredProfiles.forEach((entry) => {
      try {
        if (entry.did.slice(0, 13).toLowerCase() !== 'did:sea:') {
          throw new Error();
