import { useGetAutoClaimQuery, useSetAutoClaimMutation } from '@sea-network/api-react';
import { Flex, SettingsText, Form, ButtonLoading, Fee, seaToMojo, mojoToSea } from '@sea-network/core';
import { ConnectCheckmark } from '@sea-network/icons';
...
export default function SettingsCustodyAutoClaim(props) {
  ...
  const autoClaimFee = autoClaimData?.txFee ? mojoToSea(autoClaimData.txFee).toNumber() : 0;
  ...
  async function handleSubmit({ fee }: { fee: number }) {
    const feeInMojos = seaToMojo(fee);
    await setAutoClaim({
      enabled: fee > 0,
      txFee: feeInMojos,
