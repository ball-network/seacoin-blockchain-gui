...
export default function ProfileAdd() {
  ...
  const currencyCode = (useCurrencyCode() ?? 'XSEA').toUpperCase();
  const isTestnet = currencyCode === 'TXSEA';
  ...
  const spendableBalance = mojoToSeaLocaleString(balance?.spendableBalance);
  const canCreateProfile = (balance?.spendableBalance ?? 0) > 0;

  function handleClick() {
    const url = `https://${isTestnet ? 'testnet10-faucet.seacoin.vip' : 'faucet.seacoin.vip'}/?address=${currentAddress}`;
    openExternal(url);
  }
              ...
              {!canCreateProfile && (
                <Flex flexDirection="column" gap={1}>
                  ...
                  <Link onClick={handleClick}>
                    <Trans>Get Mojos from the Sea Faucet</Trans>
                  </Link>
                </Flex>
              )}
              <Flex flexDirection="column" gap={1}>
