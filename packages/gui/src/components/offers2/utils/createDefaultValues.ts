...
type CreateDefaultValuesParams = {
  walletType?: WalletType; // CAT or STANDARD_WALLET (XSEA), indicates whether a token or CAT has a default entry
 ...
 * for creating an XSEA offer, a CAT offer, or an NFT offer. XSEA and CAT offers will have a default entry in
...
export default function createDefaultValues(params: CreateDefaultValuesParams | undefined = {}): OfferBuilderData {
  ...
  return {
    ...
    offered: {
      ...
      nfts,
      xsea: walletType === WalletType.STANDARD_WALLET ? [{ amount: '' }] : [],
