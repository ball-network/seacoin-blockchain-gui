import { ConfirmDialog, Flex } from '@sea-network/core';
...
function getSpendableAmountUponUnlockingAssets(assetStatus: AssetStatusForOffer, assetsToBeUnlocked: PendingAsset[]) {
  ...
  for (let s = 0; s < assetsToBeUnlocked.length; s++) {
    ...
    if (lockedAsset.type === 'XSEA' && assetStatus.type === 'XSEA') {
      // assetId might be either 'XSEA' or 'UNKNOWN'(maybe fee) but here both are treated as just a XSEA spending
