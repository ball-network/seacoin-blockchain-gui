import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@sea-network/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@sea-network/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@sea-network/api-react';
import { catToMojo, seaToMojo } from '@sea-network/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedXSEA = useWatch({
    name: 'requested.xsea',
  });

  const offeredXSEA = useWatch({
    name: 'offered.xsea',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedXSEA ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('xsea', seaToMojo(amount))),
    ...(requestedTokens ?? [])
