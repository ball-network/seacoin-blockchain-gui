import { Color, Flex } from '@sea-network/core';
import { Offering, Requesting } from '@sea-network/icons';
...
import OfferBuilderXSEASection from './OfferBuilderXSEASection';
...
export default function OfferBuilderTradeColumn(props: OfferBuilderTradeColumnProps) {
  ...
  const showXSEA = !readOnly || !!xsea.length;
  ...
  const mutedXSEA = nfts.length || tokens.length;
  const mutedTokens = xsea.length || nfts.length;
  const mutedNFTs = xsea.length || tokens.length;
  ...
  return (
    <Flex flexDirection="column" gap={3}>
      ...
      >
        {showXSEA && <OfferBuilderXSEASection name={`${name}.xsea`} offering={offering} muted={mutedXSEA} />}
