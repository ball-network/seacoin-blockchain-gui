import { toBech32m, WalletType } from '@sea-network/api';
import { mojoToSea, mojoToCAT } from '@sea-network/core';
...
export default function createOfferForIdsToOfferBuilderData(
      ...
      if (asset) {
        switch (asset.walletType) {
          case WalletType.STANDARD_WALLET:
            section.xsea.push({ amount: mojoToSea(numericValue.abs()).toFixed() });
            break;
          case WalletType.CAT:
  ...
  if (fee) {
    offerBuilderData.offered.fee = [{ amount: mojoToSea(fee).toFixed() }];
  }
