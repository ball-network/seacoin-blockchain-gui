...
import { seaToMojo, catToMojo } from '@sea-network/core';
...
export default async function offerBuilderDataToOffer({
  ...
  const feeInMojos = firstFee ? seaToMojo(firstFee.amount) : new BigNumber(0);
        ...
        if (assetId.toUpperCase() === 'XSEA' || assetId.toUpperCase() === 'UNKNOWN') {
          ...
          if (idx > -1) {
            ...
            if (pendingOffers[idx].assetId.toUpperCase() !== assetId.toUpperCase()) {
              // Now we can distinguish that we have xsea spending which is only XSEA, only Fee or both XSEA and Fee
              pendingOffers[idx].assetId = 'XSEA+FEE';
            }
          } else {
            pendingOffers.push({
              type: 'XSEA',
  ...
  const xseaTasks = offeredXsea.map(async (xsea) => {
    ...
    const mojoAmount = seaToMojo(amount);
  ...
  requestedXsea.forEach((xsea) => {
    ...
    walletIdsAndAmounts[wallet.id] = seaToMojo(amount);
  });
