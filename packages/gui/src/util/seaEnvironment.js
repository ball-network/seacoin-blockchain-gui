...
const getSeaVersion = () => {
  let version = null;
  const exePath = getExecutablePath('sea');
  // first see if we can get a sea exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or sea is on the path
    try {
      version = childProcess
