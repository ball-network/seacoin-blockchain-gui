...
export function getUserDataDir(): string {
  const seaRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(seaRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const seaRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${seaRootUserDataPath}`);
  app.setPath('userData', seaRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(seaRootUserDataPath, 'Local Storage', 'leveldb');
