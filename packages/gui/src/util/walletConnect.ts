...
const log = initDebug('seacoin-gui:walletConnect');

const availableCommands = walletConnectCommands.map((command) => `sea_${command.command}`);
...
export async function processSessionProposal(
  ...
  try {
    ...
    const requiredNamespace = requiredNamespaces.sea;
    if (!requiredNamespace) {
      throw new Error('Missing required sea namespace');
    }

    const { chains, methods } = requiredNamespace;
    const chain = chains.find((item) => ['sea:testnet', 'sea:mainnet'].includes(item));
    if (!chain) {
    ...
    const accounts = fingerprints.map((fingerprint) => `sea:${instance}:${fingerprint}`);
