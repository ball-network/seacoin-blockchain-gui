import { TransactionType, WalletType } from '@sea-network/api';
import type { Transaction } from '@sea-network/api';
import { useGetWalletBalanceQuery } from '@sea-network/api-react';
import { Color, mojoToSea, mojoToCAT, blockHeightToTimestamp } from '@sea-network/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToSea(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: (walletType === WalletType.CAT ? mojoToCAT(balance) : mojoToSea(balance)).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToSea(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: walletType === WalletType.CAT ? mojoToCAT(start) : mojoToSea(start).toString(), // bignumber is not supported by react
    });
  });
