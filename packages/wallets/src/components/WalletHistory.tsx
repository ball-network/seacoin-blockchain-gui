...
import type { Row } from '@sea-network/core';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate, location) => [
    ...
    field: (row: Row, metadata) => {
      ...
      return (
        <>
          ...
          <strong>
            <FormatLargeNumber
              value={[WalletType.CAT, WalletType.CRCAT].includes(type) ? mojoToCAT(row.amount) : mojoToSea(row.amount)}
            />
          </strong>
          &nbsp;
          {metadata.unit}
...

  {
    field: (row: Row, metadata) => (
      <>
        <strong>
          <FormatLargeNumber value={mojoToSea(row.feeAmount)} />
        </strong>
        &nbsp;
        {metadata.feeUnit}
      </>
