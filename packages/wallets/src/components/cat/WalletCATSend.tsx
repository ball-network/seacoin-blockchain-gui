import { SyncingStatus, toBech32m, WalletType } from '@sea-network/api';
import { useSpendCATMutation, useFarmBlockMutation } from '@sea-network/api-react';
import {
  AdvancedOptions,
  Button,
  EstimatedFee,
...
export default function WalletCATSend(props: Props) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      throw new Error(t`Cannot send sea to coloured address. Please enter a sea address.`);
    }

    if (address.includes('sea_addr') || address.includes('colour_desc')) {
    ...
    if (address.slice(0, 16) === 'sea_addr://') {
