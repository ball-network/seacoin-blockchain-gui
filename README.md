...
Welcome to the Sea GUI repo!

Sea GUI is written in TypeScript and uses Electron/React.

This monorepo consists of the following packages:

| Package name  | Description                                                                                                |
| ------------- | ---------------------------------------------------------------------------------------------------------- |
| **api**       | JS/TS library to access the SeaCoin Blockchain RPC                                                            |
...
| **icons**     | Sea specific icons                                                                                        |
| **wallets**   | Common React components and hooks. Do not use this in you project. Will be merged to **core** package soon |

## Development

1. This repo (seacoin-blockchain-gui) must be under seacoin-blockchain repo. Please follow the [installation steps for the seacoin-blockchain](https://github.com/Chia-Network/chia-blockchain/wiki/INSTALL#install-from-source). Make sure to install from source code (git clone...).
2. Run the `sh install-gui.sh` as instructed in the previous step. This will clone the seacoin-blockchain-gui under seacoin-blockchain repo.
3. Change directory into the seacoin-blockchain-gui subdirectory.
4. Run `npm run dev:gui`

**When developing, please:**

- Only edit the code with the **Vscode editor**.
- Always have **seacoin-blockchain-gui opened as a root folder in the Vscode** and not seacoin-blockchain, or seacoin-blockchain-gui/packages/... Failing to do so will result in incorrect auto linting and auto formatting which would not go through the CI quality checks.
...
- To install `lodash` for **single** package: `npx lerna add lodash --scope=@sea-network/icons`
...
- For Sea employees: **prefix your branch with your name**, like this: `yourName/what-is-the-code-about`. This helps when cleaning up old branches.
...
Do not edit files directly in the repo, but instead please head over to our [Crowdin project](https://sea.crowdin.com/seacoin-blockchain-gui) and add/edit translations there.
...
- **`npm run dev:gui` fails to start the app without providing a reason**
  ...
  1. In your command line, please go to the `seacoin-blockchain` directory (one level up)
  2. Run `. ./activate`
  3. Run `cd seacoin-blockchain-gui`
  4. Run `npm run dev:gui` to start the app.
  5. If still does not work, please open you process manager and kill all Sea / Python related processes.
...
1. In the Sea Electron app, click `View -> Developer -> Developer tools`.
...
1. Please follow the [Install and configure the simulator](https://docs.seacoin.vip/guides/simulator-user-guide/). Do this step only once.
2. In the seacoin-blockchain directory, run this to setup the ENV variables. Use these instead of the ones mentioned in the above guide.

```
export SEA_ROOT=~/.sea/simulator/main
export SEA_SIMULATOR_ROOT=~/.sea/simulator
export SEA_KEYS_ROOT=~/.sea_keys_simulator
```

3. `. ./activate`
4. `sea start simulator`
5. `cd seacoin-blockchain-gui/packages/gui`
6. `npm run dev:skipLocales`
7. You should see your simulator wallets. You should not see your testNet / mainNet wallets.
8. Run `sea dev sim farm` as many times you want to farm some coins.
